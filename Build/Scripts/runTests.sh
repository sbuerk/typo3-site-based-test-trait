...
case ${TEST_SUITE} in
    ...
    checkRst)
        if [[ -d "Documentation" ]]; then
          ...
          SUITE_EXIT_CODE=$?
            ...
            echo ">> No documentation found. Valid."
            SUITE_EXIT_CODE=0
        fi
        ;;
    checkExceptionCodes)
    ...
    composerUpdate)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-update-${CORE_VERSION}-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} composer update
        SUITE_EXIT_CODE=$?
